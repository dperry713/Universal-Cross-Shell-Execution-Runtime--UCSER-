...
class CryptoMiddleware(BaseMiddleware):
    ...
    async def process(self, ucer: UCER, context: ExecutionContext) -> UCER:
        ...
        ucer.execution_pub_key = exec_pub.decode('utf-8') if isinstance(exec_pub, bytes) else exec_pub
            ...
            for step in ucer.steps:
                ...
                try:
                    ...
                    trace = ExecutionTrace(
                        step_id=step.step_id, adapter=adapter_name, command=step.command,
                    ...
                    if not self.sentinel.inspect_trace(trace, context):
                        ucer.status = "failed"
                        raise SecurityError(f"Sentinel Blocked Execution at step {step.step_id}: Behavioral Anomaly Detected.")

                    ucer.traces.append(trace)
                    asyncio.create_task(self.telemetry.emit_execution_metric(ucer.command_id, adapter_name, duration_ms, True))
                    
                except Exception as e:
                    ucer.status = "failed"
                    asyncio.create_task(self.telemetry.emit_execution_metric(ucer.command_id, adapter_name, 0.0, False))
                    raise
